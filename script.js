...
/* RENDER SKILLS */
const skBlock = document.getElementById('sk-block');
skBlock.innerHTML = `
    <div class="sk-heading">
        <div class="sk-eyebrow">Tech I Use</div>
        <div class="sk-h2">MY SKILLS</div>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ${skillCats.map(c => `
            <div class="sk-cat">
                <div class="sk-cat-label">${c.label}</div>
                <div class="sk-tags">
                    ${c.items.map(i => `<span class="sk-tag">${i}</span>`).join('')}
                </div>
            </div>
        `).join('')}
    </div>
...
const ro = new IntersectionObserver(es => {
    es.forEach(e => {
        if (e.isIntersecting) {
            e.target.classList.remove('reveal-hidden'); ro.unobserve(e.target)
        }
    })
}
